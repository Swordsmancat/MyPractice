   ...
   public class EnemyBaseActionState :EnemyBaseState
    {
        ...
        private void OnApplyDamageEvent(object sender, GameEventArgs e)
        {
            ...
            if (owner.TargetableObjectData.VertigoValue < 0)
            {
                if(owner.fsm.CurrentState.GetType() != owner.ChangeStateEnemy(EnemyStateType.Vertigo))
                {
                    ChangeState(ProcedureOwner, owner.ChangeStateEnemy(EnemyStateType.Vertigo));
                    return;
                }
            }

            if (owner.TargetableObjectData.TrunkValue > 0 && !owner.GetRebound && owner.GetCollider == ColliderState.None && owner.GetGP == GPAttack.None && (owner.GetBuffType == BuffType.None || owner.GetBuffType == BuffType.Tap || owner.GetBuffType == BuffType.Thump || owner.GetBuffType == BuffType.Overwhelmed))
            {
                return;
            }

            if (owner.GetCollider != ColliderState.None)
            {
