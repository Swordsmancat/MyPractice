...
//	private Rigidbody _rigidbody;
	...
	private CinemachineVirtualCamera LockVacam;
	private CinemachineVirtualCamera AimVacam;
	private int PriorityBoostAmount = 10;
	private bool boosted = false;
	private bool m_LockBoosted = false;
	private Vector3 m_DefualtPositionLock;

	private Vector3 m_HighFollowOffset = new Vector3(0, 1.5f, -4.5f);
	private Vector3 m_TopRigPosition;
	private bool m_IsHigtCamera =false;
	...
	void Start()
	{
		...
		LockVacam = behaviourManager.CameraControl_Lock.GetComponent<CinemachineVirtualCamera>();
		...
		m_DefualtPositionLock = behaviourManager.m_LookAt.position;
		m_TopRigPosition = behaviourManager.m_TopRigTransform.localPosition;
	}
    ...
    void Update()
	{
		...
		if(LockVacam != null)
        {
			...
			if (isLock)
            {
				...
				if (!m_LockBoosted)
                {
					LockVacam.Priority += PriorityBoostAmount;
					m_LockBoosted = true;
				}

            }
			else if(m_LockBoosted)
            {
				LockVacam.Priority -= PriorityBoostAmount;
				m_LockBoosted = false;
				//m_LockBoosted = false;
  ...
  //      {
	...
	//	behaviourManager.GetRigidBody.Move(speedDown * Time.deltaTime);
        ...
        {
            if (lockTarget != null && !isKnockLock)
            {
				if(horizontal !=0 ||vertical != 0)
                {
					...
					forward = (lockTarget.position - transform.position).normalized;
	...
	public void AttackRotating()
    {
        ...
        {
			Vector3 dir;
			dir = lockTarget.position - transform.position;
			dir.y = 0;
        ...
        {
			angle = behaviourManager.playerCamera.eulerAngles.y - transform.eulerAngles.y;
		}
		
		Quaternion targetQuaternion = Quaternion.Slerp(transform.rotation, Quaternion.Euler(0,
			  transform.localRotation.eulerAngles.y + angle, 0), 0.1f);
